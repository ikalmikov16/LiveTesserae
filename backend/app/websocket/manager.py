...
class ConnectionManager:
    ...
    def __init__(self):
        ...
        self.subscriptions: dict[WebSocket, set[str]] = {}  # connection -> subscribed chunk IDs
    ...
    def subscribe(self, websocket: WebSocket, chunk_ids: list[str]):
        ...
        logger.debug(f"Client subscribed to chunks: {chunk_ids}. Total: {len(self.subscriptions[websocket])}")
    ...
    def unsubscribe(self, websocket: WebSocket, chunk_ids: list[str]):
        ...
        if websocket in self.subscriptions:
            ...
            logger.debug(f"Client unsubscribed from chunks: {chunk_ids}. Remaining: {len(self.subscriptions[websocket])}")
