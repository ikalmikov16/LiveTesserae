...
async def _update_chunk_and_overview(cx: int, cy: int, x: int, y: int, tile_data: bytes | None) -> None:
    """
    ...
    try:
        ...
        chunk_image_data = await chunk_renderer.update_chunk_tile(cx, cy, x, y, tile_data)
        chunk_version = await storage.save_chunk_image(cx, cy, chunk_image_data)
        
        # Update the overview (Level 0)
        overview_image_data = await chunk_renderer.update_overview_chunk(cx, cy, chunk_image_data)
        await storage.save_mosaic_overview(overview_image_data)
        
        logger.debug(f"Background: Updated chunk ({cx}, {cy}) v{chunk_version} and overview")
...
async def save_tile(x: int, y: int, image_data: bytes) -> dict:
    ...
    try:
        ...
        _schedule_background_task(
            _update_chunk_and_overview(cx, cy, x, y, image_data)
        )
        
        logger.info(f"Saved tile {tile_id} (version {row['version']}), chunk update scheduled")
...
async def delete_tile(x: int, y: int) -> dict | None:
    ...
    if "DELETE 1" in result:
        ...
        # Fire and forget - update chunk and overview in background
        _schedule_background_task(
            _update_chunk_and_overview(cx, cy, x, y, None)
        )
