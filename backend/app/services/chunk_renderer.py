...
def _get_tile_bounds_in_chunk(local_tx: int, local_ty: int) -> tuple[int, int, int, int]:
    """
...
async def update_chunk_tile(cx: int, cy: int, tx: int, ty: int, tile_data: bytes | None) -> bytes:
    """
    ...
    if existing_chunk:
        ...
        chunk_img = Image.new(
            "RGBA",
            (CHUNK_PREVIEW_SIZE, CHUNK_PREVIEW_SIZE),
            (255, 255, 255, 255)
        )
...
async def render_chunk(cx: int, cy: int) -> bytes:
    ...
    for tx in range(start_x, min(start_x + settings.chunk_size, settings.grid_width)):
        for ty in range(start_y, min(start_y + settings.chunk_size, settings.grid_height)):
            ...
            if tile_data:
                try:
                    ...
                    logger.warning(f"Failed to render tile ({tx}, {ty}) into chunk: {e}")
...
async def update_overview_chunk(cx: int, cy: int, chunk_data: bytes) -> bytes:
    ...
    if existing_overview:
        ...
        overview_img = Image.new(
            "RGBA",
            (MOSAIC_PREVIEW_SIZE, MOSAIC_PREVIEW_SIZE),
            (255, 255, 255, 255)
        )
        ...
        for cy in range(chunks_per_row):
            ...
            if chunk_data:
                try:
                    ...
                    logger.warning(f"Failed to render chunk ({cx}, {cy}) into overview: {e}")
