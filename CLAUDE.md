...
**AWS status (as of Aug 2026): the original AWS account (199264265773) is permanently closed** — every identifier in `backend/task-def.json` and both `.env.deploy.example` flows (ECR repo, RDS endpoint, S3 buckets, CloudFront distributions, IAM roles) is defunct. **The redeploy is planned as a different architecture — see Part 1 of `.cursor/plans/PLAN.md`:** one Lightsail box running Postgres + the app + Caddy, with `STORAGE_MODE=local`. No ECS, no ALB, no RDS, no S3, no CloudFront. That makes `task-def.json`, `register-task-def.sh` and the ECR/ECS half of `deploy.sh` ECS-only baggage to delete once the box is live (Phase 8 of the plan). The old S3/CloudFront/RDS setup guides are in `.cursor/plans/archive/` and should not be followed.

## Plans & project docs

- **`.cursor/plans/PLAN.md` is the single source of truth for planned work** (gitignored, local only). Consolidated Aug 16 2026 from sixteen separate plan files. Three parts: **Deploy** (the only active work — one Lightsail box running Postgres + app + Caddy), **Backlog** (editor bugs, mobile, small stuff), **Reference** (measured numbers, verified-bug history, rejected claims, the credential leak).
- **Read PLAN.md before planning anything.** Its Reference section exists specifically so that verified findings and already-rejected ideas aren't rediscovered — including why horizontal scaling, Redis, Aurora Serverless and several managed Postgres tiers were ruled out.
- When asked to plan a feature, add it to PLAN.md rather than creating a new file. If a feature genuinely needs its own document, keep PLAN.md as the index that points to it. House style: goal, current state, phased steps with file paths, effort estimates.
- **`.cursor/plans/archive/` holds 39 superseded documents.** They are design history, not instructions — several contain stale progress trackers listing work that shipped long ago (and one that never existed: there is no Lambda chunk renderer). Pull one back out only for detail PLAN.md deliberately summarised, such as the phased touch-support breakdown in `mobile-support.md`.
...
- There is still no Save button — a tile only reaches the server when the editor closes or another tile is clicked, so "live" means per-close, not per-stroke. See the Backlog section of `.cursor/plans/PLAN.md` for the rest of the editor input bugs.
