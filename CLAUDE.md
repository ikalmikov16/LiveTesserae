...
- **`.cursor/plans/deployment.md` is the deploy playbook** — one Lightsail box running Postgres + the app + Caddy. Self-contained: account setup, phases 0–8, verification, and an appendix of hosting options already ruled out (so Aurora Serverless, Neon, Redis and horizontal scaling aren't rediscovered). The older `aws-deployment.md`, `deployment-v2.md` and `step12*` docs are in `archive/` and should not be followed.
- Other plans are design history and still-relevant backlog: `pre-redeploy-hardening.md` (Phases 1–2 implemented Aug 16 2026; §1.8 editor bugs still open), `mobile-support.md` (touch pan/pinch for MosaicCanvas, phased), `production-hardening.md` + `hardening-tests.md` (mostly implemented), `IMPLEMENTATION_ORDER.md` / `OVERVIEW.md` (original roadmap; progress tracker is stale — it lists a Lambda chunk renderer that never existed).
